    ...
    def run(self, encoding=enco):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            try: 
                ...
                logging.error('Failed to start server %s at address: '%self._name + 
                    ...
                    with conn: # close the connection after this code is executed:
                        try:
                            ...
                            try:
                                ...
                                logging.error('Python server %s: client terminated connection before message was sent.'%self._name +
                            ...
                            try:
                                ...
                                logging.warning('Python server %s: client terminated connection before receive.\n'%self._name+str(e))
                            self.ts['received'].append(time.time() - self.ts['connect'][-1] - self.ts['sent'][-1])
                            self.ts['disconnect'].append(time.time())
                        except IndexError as e: 
                            logging.error('Server %s msg queue was emptied before msg could be sent.\n'%self._name+str(e))
